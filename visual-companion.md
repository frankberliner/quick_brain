...
> Loaded on-demand by `SKILL.md` (quickbrain) when the user has accepted the visual companion.
...
**Note:** Pass the project root as `--project-dir` so mockups persist in `.quickbrain/brainstorm/` (or whatever the server resolves to under that root) and survive server restarts. Without it, files go to `/tmp` and get cleaned up. Remind the user to add the persistence directory (e.g., `.quickbrain/` or `.superpowers/`) to `.gitignore` if it's not already there. The exact directory name comes from the server scripts in `scripts/`; whichever path is reported in the startup JSON is the one to gitignore.
...
If the session used `--project-dir`, mockup files persist under the project's brainstorm directory for later reference. Only `/tmp` sessions get deleted on stop.
